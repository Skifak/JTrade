<script>
  ...
  const sections = [
    ...
    { id: 'journal-day',   icon: '📓', title: 'Дневник дня' },
...
<div class="guide">
  ...
  <main class="guide-content">
    ...
    <section id="about" class="guide-section hero">
      ...
      <div class="hero-grid">
        ...
        <div class="hero-card">
          ...
          <p>KPI и heatmaps во вкладке «Статистика»; «Аналитика» — маяки, дорожная карта, советы с оригиналами постов (Виктор / Руфат), фокус недели с прогрессом и сравнение недель.</p>
        </div>
      </div>
    </section>
    ...
    <section id="quickstart" class="guide-section">
      ...
      <p class="lead">
        Всё локально — без регистрации и своего сервера. Данные хранятся только на этом устройстве (в браузере или в приложении).
        Перенести журнал на другой компьютер можно через <em>Экспорт</em> в профиле.
      </p>
      ...
      <ol class="steps">
        ...
        <li>
          ...
          <p>В профиле вкладка <strong>«Правила и ограничения»</strong>: лимиты, cooldown, anti-martingale, цели и напоминания — плюс карточки всех проверок гейта и HUD.</p>
        </li>
      </ol>
    ...
    <section id="profile" class="guide-section">
      ...
      <p class="lead">
        ...
        фичи приложения опираются на него. Риск, лимиты, цели и расшифровка правил — вкладка <strong>«Правила и ограничения»</strong>.
      </p>
      ...
      <h3 class="sub-h">Счета журнала: чистая история и импорт</h3>
      <p class="lead-sub" style="margin: 0 0 12px;">
        В <button class="link-btn" on:click={openProfile}>Профиле</button> → вкладка <strong>«Настройка счёта»</strong>: переключатель активного счёта, создание нового, импорт в текущий, экспорт.
        У каждого счёта свой набор сделок, профиль, глоссарий, дневник дня и настройки — как отдельные «коробки».
      </p>
      <div class="cols-2">
        <div class="info-card">
          <h4>Счёт с чистой историей</h4>
          <ul>
            <li>Создаётся кнопкой <strong>«Создать счёт с пустой историей»</strong> (первый запуск или «Создание счёта» в профиле).</li>
            <li>Таблица сделок пустая; капитал, валюта и риск задаёшь сам в профиле нового счёта.</li>
            <li><strong>Импорт файла с сделками в такой счёт отключён</strong> — приложение не смешивает «нулевой старт» с заливкой истории из MT5/ZIP; ведёшь учёт только новыми записями и ручными правками.</li>
            <li>Зачем: отдельный трек под новую стратегию, челлендж или дисциплину без переноса старого шума из терминала.</li>
          </ul>
        </div>
        <div class="info-card">
          <h4>Счёт с импортом</h4>
          <ul>
            <li>Создаётся через <strong>«Создать счёт с импортом сделок»</strong>: файл HTML отчёта MT5, JSON сделок или ZIP бандла этого приложения.</li>
            <li>Отображаемое <strong>имя счёта</strong> в журнале задаёшь ты; название счёта у брокера из файла может показываться в карточке импорта.</li>
            <li>В таблице у строк часто есть значок источника (например импорт истории MT5). В профиле доступны <strong>история импортов</strong> и откат сделок конкретного импорта.</li>
            <li>На уже созданный <em>импорт-счёт</em> можно снова накатывать импорт через «Импорт в текущий счёт» — для счёта с чистой историей эта операция недоступна.</li>
          </ul>
        </div>
      </div>
    ...
    <section id="journal-day" class="guide-section">
      <h2><span class="num">📓</span>Дневник дня</h2>
      <p class="lead">
        Это <strong>не</strong> таблица сделок. Отдельный календарный дневник: настроение, план на сессию, чек-лист (шаблон настраивается),
        ревью и уроки. Записи привязаны к датам и живут рядом с данными активного счёта, но отделены от строк «Открытые / Закрытые».
      </p>
      <div class="info-card" style="margin-bottom: 14px;">
        <h4>Где открыть в приложении</h4>
        <ul>
          <li>Верхняя строка вкладок под заголовком «Журнал»: нажми <strong>«Дневник»</strong> — она стоит между <strong>«Аналитика»</strong> и <strong>«Глоссарий»</strong>.</li>
          <li>Вечером может появляться напоминание «закрой день» — кнопка <strong>«Открыть дневник»</strong> переключает на ту же вкладку.</li>
          <li>Из раздела «Аналитика» ссылка «в дневник» тоже ведёт сюда — фокус недели и графики опираются на эти записи.</li>
        </ul>
      </div>
      <div class="cols-2">
        <div class="info-card">
          ...
          <ul>
            ...
            <li>Свой чек-лист дня (KZ, лимит сделок, правила) — в шестерёнке настроек блока дневника можно менять строки шаблона.</li>
          </ul>
        </div>
        <div class="info-card">
          <h4>Как пользоваться</h4>
          <ul>
            <li>Выбери дату в календаре — редактор справа, слева список дней, где уже есть содержание.</li>
            <li>Быстрые вставки плана — кнопки под полем плана на день.</li>
          </ul>
        </div>
      </div>
    </section>
    ...
    <section id="glossary-media" class="guide-section">
      ...
      <p class="lead">
        Вкладка <strong>Глоссарий</strong> — карточки терминов по категориям (ICT и свои). Текст карточек хранится локально вместе с данными счёта;
        к каждому термину и к <strong>закрытым</strong> сделкам можно прикреплять скриншоты.
      </p>
      <div class="cols-2">
        <div class="info-card">
        ...
        <div class="info-card">
          ...
          <ul>
            <li><strong>Tauri (десктоп):</strong> папка данных приложения (например AppData на Windows) — внутри каталог вложений для глоссария и сделок.</li>
            <li><strong>Веб-режим:</strong> файлы изображений лежат в отдельном хранилище браузера (IndexedDB), чтобы не забивать лимит основных настроек.</li>
          </ul>
        </div>
        <div class="info-card big">
    ...
    <section id="killzones" class="guide-section">
      ...
      <p class="lead">
        Сессия (killzone) определяется по <strong>времени открытия</strong> сделки в часовом поясе из «Параметров журнала»
        (по умолчанию восточное время США, переход летнее учитывается). Значение попадает в карточку сделки и участвует в pre-trade проверках и отчётах.
      </p>
      ...
      <div class="cols-2">
        <div class="info-card">
          ...
          <ul>
            ...
            <li><b>Ключевые уровни</b>: одна строка — цена и короткая подпись (PDH, Asia low, FVG…).</li>
          </ul>
        </div>
        <div class="info-card">
    ...
    <section id="pre-trade" class="guide-section">
      ...
      <p class="lead">
        Когда жмёшь «Сохранить», приложение прогоняет сделку через проверку правил из профиля и показывает нарушения двух типов:
      </p>
      ...
      <p class="lead-sub">
        Превью нарушений видно в форме <strong>до</strong> сохранения. Кнопка «Сохранить» меняет стиль:
        обычная (всё чисто) → с предупреждением → с акцентом на риске, если есть жёсткие блокировки.
      </p>
    </section>
      ...
      <div class="cols-2">
        <div class="info-card big">
          ...
          <ul>
            ...
            <li>Сохраняется между перезапусками приложения и перезагрузкой страницы</li>
          </ul>
...
# Это комментарий — игнорируется</pre>
    ...
    <section id="live-prices" class="guide-section">
      ...
      <p class="lead">
        Никаких ключей API с твоей стороны: используются открытые потоки бирж по WebSocket.
      </p>
      ...
      <div class="cols-2">
        <div class="info-card">
          ...
          <ul>
            <li>Публичные каналы котировок и ленты сделок (Binance / Kraken).</li>
    ...
    <section id="analytics" class="guide-section">
      ...
      <p class="lead">
        Вкладка <strong>Аналитика</strong> не дублирует «Статистику»: здесь обзор <em>процесса</em> — маяки состояния, дорожная карта привычек,
        короткие советы и полные оригиналы постов двух авторов (без облака и без LLM), сравнение недель и привязка дневника к результату.
      </p>
      ...
      <div class="cols-2">
        <div class="info-card">
          <h4>Подвкладки</h4>
          <ul>
            <li><strong>Обзор</strong> — сводный заголовок, мини-метрики, маяки, дорожная карта и блок «База советов» (с отсылками к источникам по ⓘ).</li>
            <li><strong>Неделя</strong> — сравнение с прошлой неделей: сделки, дневник, PnL, графики.</li>
            <li><strong>Фокус недели</strong> — один предложенный процессный приоритет на календарную неделю; кнопка «Принять» фиксирует выбор на эту неделю. Показываются понятное описание, метрика успеха, черновая оценка текущей недели, <strong>полоска прогресса</strong> и короткая расшифровка цифр. После смены недели принятый фокус уходит в <strong>историю</strong> с итогом.</li>
            <li><strong>Дневник и PnL</strong> — насколько дни с записями совпадают с торговым результатом (скользящее окно).</li>
            <li><strong>Сетапы</strong> — срез по стратегиям и сетапам из закрытых сделок.</li>
            <li><strong>Контекст</strong> — теплокарты по времени суток и сессиям (в часовом поясе killzone из параметров журнала).</li>
            <li><strong>Справка</strong> — ответы на вопросы по этому экрану.</li>
          </ul>
        </div>
        <div class="info-card">
          <h4>Устройство и счета</h4>
          <ul>
            <li>Все расчёты выполняются на устройстве; в облако журнал не отправляется.</li>
            <li>Советы и маяки строятся из твоих сделок, дневника и настроек профиля.</li>
            <li>Подвкладки аналитики и зафиксированный фокус недели привязаны к <strong>активному счёту</strong> так же, как таблица сделок — при переключении счёта контекст другой.</li>
            <li>Если приложение открыто в браузере, данные счёта обычно лежат в локальном хранилище браузера; в десктопной сборке — в хранилище приложения.</li>
          </ul>
        </div>
      </div>
    </section>
    ...
    <section id="data" class="guide-section">
      ...
      <div class="cols-3">
        <div class="info-card">
          <h4>💾 Хранение</h4>
          <ul>
            <li>Основные данные счёта — в локальном хранилище браузера; картинки к сделкам и глоссарию — отдельно (см. раздел про вложения).</li>
            <li>Объём типично хватает на большой журнал; если лимит хранилища исчерпан, появится подсказка — сделай экспорт и освободи место.</li>
          </ul>
        </div>
        <div class="info-card">
    ...
    <section id="faq" class="guide-section">
      ...
      <details class="faq-item">
        ...
        <p>Сумма прибыли только по сделкам <strong>без</strong> зафиксированных нарушений правил из pre-trade. Это условная линия «если бы всегда соблюдал план».</p>
      </details>
